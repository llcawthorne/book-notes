...
func TestRun(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expErr != nil {
				if err == nil {
					t.Errorf("Expected error: %q, Got 'nil' instad.", tc.expErr)
					return
				}
...
func setupGit(t *testing.T, proj string) func() {
	t.Helper()
	gitExec, err := exec.LookPath("git")	// fail fast if we don't have git
	if err != nil {
		t.Fatal(err)
	}
...
func TestHelperProcess(t *testing.T) {
	...
	// This was git, but I was faking out git with ls
	if os.Args[2] == "ls" {
...
func TestRunKill(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// we want the timeout mock to give time to send signals
			...
			// no errors, so we expect our SIGQUIT
			case <-ignSigCh:
			}
		})
	}
