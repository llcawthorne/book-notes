...
func createTempDir(t *testing.T,
	...
	// In old Go, test helpers could return a cleanup function to defer for
	// cleanup after a test. In Go 1.14 or later, you would normally use
	// t.Cleanup to register the cleanup function instead of returning one
...
func TestRunDelExtension(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// We can't verify exact output since log lines include time/date
			// We know there will be one more line of output than the number
			// of files deleted (one blank line, plus one per file deleted)
