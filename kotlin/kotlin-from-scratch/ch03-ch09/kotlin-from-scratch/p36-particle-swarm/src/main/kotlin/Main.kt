...
/*
 * Optimize a Multivariate Function with a Particle Swarm
 *
 * Particle swarm optimization can be used for a lot of the same problems
 * as genetic algorithms, but they are conceptually simpler and easier to 
 * implement, they normally involve fewer parameters, and they tend to converge
 * quicker on the global optima. At their core, they take three key steps:
 *
 * 1. Initialize the position and velocity of the particles.
 * 2. Proceed through the time steps, and update particle velocities based on
 *    the current velocity, the best-so-far individual position, and the global
 *    best position identified by the swarm up to that time step.
 * 3. Update the current position by moving to a better position.
 *
 * These steps are repeated either a set number of times or until a condition is
 * met.
 *
 */
...
val xBbounds = arrayOf(
    doubleArrayOf(-512.0, 512.0),
    doubleArrayOf(-512.0, 512.0)
)
val xRange = doubleArrayOf(
    xBbounds[0][1] - xBbounds[0][0],
